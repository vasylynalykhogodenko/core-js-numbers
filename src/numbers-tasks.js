...
function toNumber(value, def = 0) {
  ...
  return parsedValue ? def : parsedValue;
}
toNumber(null, 0);
toNumber('test', 0);
toNumber('1', 0);
...
function numberToStringInBase(number, base) {
  ...
  const convertDigit = (num) => digits[num % base];
  const convertRest = (num) => Math.floor(num / base);

  return number === 0 ? '' : convertRest(number) + convertDigit(number);
}
